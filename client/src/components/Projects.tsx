...
const Project: React.FC = () => {
  ...
  return (
    <section
      id="projects"
      className="relative flex flex-col items-center justify-center pt-25 px-6 sm:px-[clamp(1rem,11.46vw,200px)] scroll-auto gap-20 font-[michroma]"
    >
      {/* Header */}
      ...
      <div className="flex flex-col gap-15 md:gap-16">
        {featuredProjects.map((project, index) => (
          <div key={index} className="">
            <div
              className={`flex flex-col gap-10 md:gap-5 md:flex-row items-center md:items-start lg:items-center justify-between w-full group h-full rounded-3xl md:text-left transition-all duration-500 transform backdrop-blur-xs ${
                isDarkMode ? "" : ""
              }`}
            >
              ...
              <div className="relative overflow-hidden rounded-3xl flex items-center  justify-center md:w-1/2">
                {project.image ? (
                  <img
                    src={project.image}
                    alt={project.title}
                    className="object-center object-contain group-hover:scale-102 transition-transform duration-500"
                  />
                ) : (
